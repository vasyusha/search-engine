...
//8.Find document, get id and relevance
vector<pair<int, int>>FindDocument( const vector<pair<int, vector<string>>>& documents,
    ...
    for( const auto& document : documents ) {
        ...
        if( relevant > 0 ) {
            rezult.push_back({document.first, relevant});
        }
    }
...
int main() {
    ...
    for( auto& [id, value] : FindDocument(databases, stop_words, query) ) {
